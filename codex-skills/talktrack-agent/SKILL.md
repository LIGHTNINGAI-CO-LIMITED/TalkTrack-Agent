...
- Enforce current-IVR intent ownership before every `updateSceneList`, copy, import, or delivery. Read `GET /ivrIntent/findList/{ivrId}` and build the authoritative positive intent-ID set for the exact target IVR. Every positive numeric intent reference in backend `sceneList.nodeList[].intentList`, frontend `sceneListFrontend.nodeList[].intentList`, graph `customData.intentList`, and related interrupted-intent rows must belong to that set; only reserved system IDs such as `-1` and `-2` bypass this ownership check. A structurally valid but foreign ID is a P0 runtime-and-copy blocker: it can stop routing before the model is called and can make IVR copy fail. Never copy a source IVR's positive intent IDs into a target IVR. Re-read the target intent list and remap by exact intent name; if a unique target mapping cannot be proven, stop instead of guessing, deleting the route, or calling `updateSceneList`.
...
- When configuring, checking, or importing a prompt that contains `intent`, read `references/intent-usage-rules.md` and `references/output-format-constraint-v0.1.md` first. Enforce business intent semantics against the prompt and IVR ports/mappings, while leaving result serialization to the platform output-format constraint.
- For new domestic smart Agents, set `llmNodeOutputFormatConstraintEnabled=1` and the verified canonical `llmNodeOutputFormatConstraintPrompt` in backend node, frontend node, and graph `customData`. Keep `{Agentintentlist}` literal and present exactly once. The Debug UI only hides or shows the editor; API read/write and readback remain required. Do not append a second output-format section, static candidate intent list, JSON-tail rule, or `{Agentintentlist}` to `llmNodeModelConfig.prompt`.
- When updating an existing domestic smart Agent, migrate legacy prompt-owned output rules automatically only when they are in an exact canonical block, explicit start/end markers, or a clearly headed Markdown section. Preserve business triggers, collection rules, terminal ownership, compliance, and intent examples. If format instructions remain mixed into business content after safe removal, stop before `updateSceneList` for manual review. For overseas smart Agents, preserve existing output-constraint fields until overseas behavior is separately verified.
...
- For sequential qualification collection, smart information collection must define both fields and conversation behavior. If the source prompt has a fixed field order, preserve that order in the prompt and field descriptions. The Agent asks one question at a time, advances only after a valid answer to the current field, and must not jump forward, go backward, or repeat a skipped field. Interruptions, questions, objections, small talk, and explanation requests are not valid answers to the current field; answer briefly, then return to the interrupted question. Explicit `跳过` / `不想答` / `不清楚` is a valid field status and should advance to the next field without repeated pursuit.
- When a sequential collection prompt needs terminal outcomes, keep the business conditions and concise `采集成功` / `采集失败` examples, but do not add a separate serialization contract to the business prompt. The platform output-format constraint requires one result JSON and merges `intent`, custom inline `param`, and `waitAsk` when needed together. Inline `param` field names must still exactly match `变量管理 -> 对话字段` / `llmNodeCollectParamList`.
...
6. For smart Agent nodes, clone a known-good scene graph shape from an existing IVR, then replace only business fields. Before writing a cloned graph, read source and target `/ivrIntent/findList/{ivrId}` catalogs and remap every positive traditional-route intent ID by exact intent name to the target IVR; never preserve source positive IDs.
7. If the prompt contains `intent`, read `references/intent-usage-rules.md` and `references/output-format-constraint-v0.1.md`; verify jump intents come from current node ports and descriptions, terminal labels match their mapped nodes, non-jump turns may remain in-node with empty or concrete stay intent, and neither business prompt nor platform constraint emits `兜底`. Also verify any traditional numeric route IDs embedded beside the smart node belong to the current IVR.
...
9. Import prompt Markdown as UTF-8. On new or updated domestic smart Agents, remove only safely bounded legacy output-format blocks, keep the business prompt free of `{Agentintentlist}`, and enable the platform constraint. If the migrated prompt is under 14,500 characters, write it unchanged; only compact when it is 14,500+ characters or a readback-verified write fails with `话术场景信息异常`. Treat older 10,000-character guidance as obsolete.
10. Verify with `/ivr/findSceneList/{ivrId}` and run canvas-save validation: open `/script-graph?ivrId=<ivrId>` and confirm the page can save/update, or document why only simulated page-save validation was possible. Simulated validation must reject malformed frontend intent rows or foreign positive intent IDs. For domestic smart Agents, also validate `llmNodeOutputFormatConstraintEnabled`, constraint text, and one literal `{Agentintentlist}` across backend, frontend, and graph even when the Debug UI is hidden. Confirm business Prompt contains no duplicate platform-format block. Do not report success from API readback alone when the page-save path is unverified.
...
- `output-format-constraint-migrate`: for an authorized domestic smart-Agent update, enable the platform output constraint, remove only safely bounded legacy format sections from the business prompt, and validate backend/frontend/graph readback. Stop when legacy format rules are interwoven with business logic.
...
- `intent-port-check`: verify intent labels, graph ports, terminal nodes, backend / frontend / graph consistency, and current-IVR ownership for every positive numeric route reference; confirm `兜底` appears only as a system fallback/default port when present, never as a prompt-emitted intent.
...
- Use `scripts/create_doushen_real_prompt_ivr.py` for "create a new IVR from a stable template + import a UTF-8 prompt" tasks when its parameters fit. It validates the token, resolves domestic/overseas, clones the template graph, remaps current-IVR intent IDs, rejects unsafe routes, forces `闪电26BMoE-fast` (`id=55`), migrates safely bounded legacy output-format blocks for domestic nodes, writes the platform constraint in all three copies, and reports constraint/readback status. Domestic defaults to enabled; `--disable-output-format-constraint` explicitly disables it while retaining the canonical text; overseas preserves existing fields until separately verified.
...
- Domestic smart-Agent output-format state lives at `llmNodeOutputFormatConstraintEnabled`; the canonical platform prompt lives at `llmNodeOutputFormatConstraintPrompt`. Keep both fields consistent across backend, frontend, and graph, and keep `{Agentintentlist}` literal exactly once in the enabled canonical constraint.
...
- Treat `/ivrIntent/findList/{ivrId}` as the ownership authority for positive numeric traditional-route IDs. Validate backend, frontend, graph custom data, and interrupted-intent references against the same target IVR catalog before write and after readback. A positive ID that exists only in another IVR/company record is not reusable even when its label looks correct.
...
Read `references/output-format-constraint-v0.1.md` before creating or migrating a domestic smart Agent, or when auditing `llmNodeOutputFormatConstraintEnabled`, `llmNodeOutputFormatConstraintPrompt`, `{Agentintentlist}`, or duplicate prompt-owned format rules.
