param(
    [string]$Repo = "LIGHTNINGAI-CO-LIMITED/TalkTrack-Agent",
    [string]$Branch = "main",
    [string]$SkillRoot = "$env:USERPROFILE\.codex\skills\talktrack-agent",
...
function Write-Step {
    ...
    Write-Host "[talktrack-agent-bootstrap] $Message"
}
...
$workDir = Join-Path $env:TEMP "talktrack-agent-bootstrap-$stamp"
$zipPath = Join-Path $workDir "talktrack-agent-main.zip"
...
$source = Get-ChildItem -Path $extractDir -Directory |
    Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName "codex-skills\talktrack-agent\SKILL.md") } |
    Select-Object -First 1

if (-not $source) {
    throw "Downloaded archive does not contain codex-skills\talktrack-agent\SKILL.md"
}

$skillSource = Join-Path $source.FullName "codex-skills\talktrack-agent"
...
Copy-Item -LiteralPath (Join-Path $skillSource "*") -Destination $SkillRoot -Recurse -Force
Write-Step "installed latest files"
